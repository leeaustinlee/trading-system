        ...
        return """
                ...
                <body>
                  ...
                  <script>
                    ...
                    function renderSafety(s) { const vals = s ? Object.values(s).filter(v => typeof v === 'number') : []; const warn = vals.some(v => v > 0); $('#safetyMetrics').innerHTML = '<h2 class="section-title">Safety Metrics</h2><div class="'+(warn?'status-warning':'status-ok')+'">'+(warn?'WARNING':'SAFE')+'</div>' + card({forbiddenCounters: vals.length ? vals.join(' / ') : '0', readOnly:'true'}); }
